...
// Based on leds-an30259a.c
//
// Datasheet:
// https://datasheet.lcsc.com/szlcsc/Shanghai-Awinic-Tech-AW2013DNR_C252440.pdf
...
static void aw2013_chip_disable(struct aw2013 *chip)
{
	...
	ret = regulator_disable(chip->vdd_regulator);
	if (ret) {
		dev_err(&chip->client->dev,
...
static int aw2013_chip_enable(struct aw2013 *chip)
{
	...
	ret = regulator_enable(chip->vdd_regulator);
	if (ret) {
		dev_err(&chip->client->dev,
...
static int aw2013_probe(struct i2c_client *client)
{
	...
	chip->vdd_regulator = devm_regulator_get(&client->dev, "vdd");
	ret = PTR_ERR_OR_ZERO(chip->vdd_regulator);
	if (ret) {
		if (ret != -EPROBE_DEFER)
			dev_err(&client->dev,
